...
def cucu():  # list of all interface signals, just to help You find the names
    veri.force('tb.baudrate','0')
    veri.force('tb.control','0')
    veri.force('tb.nreset','0')
    veri.force('tb.read_rx','0')
    veri.force('tb.rxd','0')
    veri.force('tb.txdata','0')
    veri.force('tb.write_tx','0')
    rx_valid = logs.peek('tb.rx_valid')
    rxdata = logs.peek('tb.rxdata')
    status = logs.peek('tb.status')
    tx_empty = logs.peek('tb.tx_empty')
